...
public class SnakerController extends Controller {
	...
	private SnakerEngineFacets facets = SnakerEngineFacets.facets;
    ...
    public void doAddActor() {
        List<Task> tasks = facets.getEngine().query().getActiveTasks(new QueryFilter().setOrderId(getPara("orderId")));
        for(Task task : tasks) {
            if(task.getTaskName().equalsIgnoreCase(getPara("taskName")) && StringUtils.isNotEmpty(getPara("operator"))) {
                facets.getEngine().task().addTaskActor(task.getId(), getPara("operator"));
            }
        }
        renderJson("success");
    }
	 ...
	 */
	public void exec() {
		facets.execute(getPara("taskId"), ShiroUtils.getUsername(), null);
	...
	public void reject() {
		String error = "";
		try {
			facets.executeAndJump(getPara("taskId"), ShiroUtils.getUsername(), null, null);
		} catch(Exception e) {
			error = "?error=1";
		}
	...
	public void undo() {
		...
		try {
			facets.getEngine().task().withdrawTask(getPara("taskId"), ShiroUtils.getUsername());
	...
	public void ccread() {
		...
		String[] assignees = new String[list.size()];
		list.toArray(assignees);
		facets.getEngine().order().updateCCStatus(getPara("id"), assignees);
